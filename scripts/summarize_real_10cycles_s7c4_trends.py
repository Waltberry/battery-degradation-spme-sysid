...
# =====================================================
"""
summarize_real_10cycles_s7c4_trends.py

Purpose
-------
Combine chunked S7_C4 real-data CT-ID results over 10 consecutive discharge cycles.

Expected input folders
----------------------
results/real_cycle_ctid_state_order_grid/
    real10_S7_C4_cycle_<cycle>_chunk_<chunk>_<n>seeds_<seed0>_to_<seed1>_dt_1.0_bins_100/
...
Outputs
-------
results/real_10cycle_s7c4_summary/
results/figures/real_10cycle_s7c4_summary/
results/tables/real_10cycle_s7c4_summary/
"""
...
OUT_DIR = PROJECT_DIR / "results" / "real_10cycle_s7c4_summary"
FIG_DIR = PROJECT_DIR / "results" / "figures" / "real_10cycle_s7c4_summary"
TABLE_DIR = PROJECT_DIR / "results" / "tables" / "real_10cycle_s7c4_summary"

OUT_DIR.mkdir(parents=True, exist_ok=True)
FIG_DIR.mkdir(parents=True, exist_ok=True)
TABLE_DIR.mkdir(parents=True, exist_ok=True)

RUN_PREFIX = "real10_S7_C4_cycle_"
...
print("=" * 100)
print("REAL 10-CYCLE S7_C4 SUMMARY")
print("=" * 100)
...
plt.ylabel("RMSE [V]")
plt.title("S7_C4 real-data fit quality across 10 discharge cycles")
plt.legend(loc="best")
plt.tight_layout()
...
plt.xlabel("Discharge cycle index")
plt.ylabel("RMSE [V]")
plt.title("All S7_C4 RMSE values across cycles")
plt.legend(loc="best")
plt.tight_layout()
...
for param in PARAMETER_COLS:
    ...
    plt.xlabel("Discharge cycle index")
    plt.ylabel(param)
    plt.title(f"Best-RMSE estimate of {param} vs cycle, S7_C4")
    plt.tight_layout()
...
plt.title("Normalized best-RMSE physical parameter trends, S7_C4")
...
for param in PARAMETER_COLS:
    ...
    plt.xlabel("Discharge cycle index")
    plt.ylabel(param)
    plt.title(f"{param} distribution across multistart seeds by cycle, S7_C4")
    plt.tight_layout()
...
for beta_col in beta_cols:
    ...
    plt.xlabel("Discharge cycle index")
    plt.ylabel(beta_col)
    plt.title(f"Best-RMSE output coefficient {beta_col} vs cycle, S7_C4")
    plt.tight_layout()
...
if {"rank_phi_raw", "ncols_phi_raw"}.issubset(df_best_by_cycle.columns):
    ...
    plt.xlabel("Discharge cycle index")
    plt.ylabel("Rank")
    plt.title("Best-run output feature rank vs cycle, S7_C4")
    plt.legend(loc="best")
    plt.tight_layout()
...
if {"rank_X_raw", "ncols_X_raw"}.issubset(df_best_by_cycle.columns):
    ...
    plt.xlabel("Discharge cycle index")
    plt.ylabel("Rank")
    plt.title("Best-run fitted state-trajectory rank vs cycle, S7_C4")
    plt.legend(loc="best")
    plt.tight_layout()
...
if "cond_phi_raw" in df_best_by_cycle.columns:
    ...
    plt.title("Best-run output feature condition number vs cycle, S7_C4")
    plt.tight_layout()
...
if "cond_X_raw" in df_best_by_cycle.columns:
    ...
    plt.title("Best-run state trajectory condition number vs cycle, S7_C4")
    plt.tight_layout()
...
if len(df_response_manifest):
    for _, best in df_best_by_cycle.iterrows():
        ...
        plt.title(f"S7_C4 cycle {cycle}: measured vs estimated, seed={seed}")
        plt.legend(loc="best")
        plt.tight_layout()
        ...
        plt.title(f"S7_C4 cycle {cycle}: residual, seed={seed}")
        plt.tight_layout()
...
# =====================================================
print("\n" + "=" * 100)
print("REAL 10-CYCLE S7_C4 SUMMARY COMPLETE")
print("=" * 100)
