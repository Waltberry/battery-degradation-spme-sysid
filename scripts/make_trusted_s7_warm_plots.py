...
"""
make_trusted_s7_warm_plots.py

Trusted post-processing plots for S7_C4 warm-continuation run.
...
Writes:
    results/figures/trusted_s7_warm_review/
    results/tables/trusted_s7_warm_review/
...
FIG_DIR = PROJECT_DIR / "results/figures/trusted_s7_warm_review"
TAB_DIR = PROJECT_DIR / "results/tables/trusted_s7_warm_review"
...
def main():
    ...
    coverage_lines = []
    coverage_lines.append("S7_C4 warm-continuation trusted review")
    ...
    coverage_lines.append("- Low rank_X_raw means state trajectory is poorly excited / poorly identifiable.")
    ...
    # Save cleaned copies
    # ------------------------------------------------------------
    df_sum.to_csv(TAB_DIR / "s7_all_cycles_summary_clean.csv", index=False)
    df_best.to_csv(TAB_DIR / "s7_all_cycles_best_runs_clean.csv", index=False)
    ...
    plt.title("S7_C4 warm continuation: best vs median RMSE")
    plt.legend(loc="best")
    savefig(FIG_DIR / "01_rmse_best_vs_median_logscale.png")

    # ------------------------------------------------------------
    # Plot 2: Best RMSE only, linear scale, clipped y-axis
    # ------------------------------------------------------------
    ...
    plt.ylim(0, ymax)
    plt.title("S7_C4 warm continuation: best RMSE only")
    ...
    plt.xlabel("Cycle index")
    plt.ylabel("Percent [%]")
    plt.title("S7_C4 warm continuation: fit quality")
    plt.legend(loc="best")
    ...
    plt.title("S7_C4 warm continuation: optimizer success/failure count")
    plt.legend(loc="best")
    ...
    plt.title("S7_C4 warm continuation: failed starts per cycle")
    ...
    plt.title("S7_C4 warm continuation: raw rank diagnostics")
    plt.legend(loc="best")
    savefig(FIG_DIR / "06_rank_diagnostics.png")

    # ------------------------------------------------------------
    # Plot 7: Physical parameters, absolute values
    # ------------------------------------------------------------
    param_cols = [
        "alpha_n_hat",
        "alpha_p_hat",
    ...
    plt.title("S7_C4 warm continuation: absolute fitted dynamic/gain parameters")
    plt.legend(loc="best", fontsize=8)
    savefig(FIG_DIR / "07_absolute_dynamic_gain_parameters.png")

    # ------------------------------------------------------------
    # Plot 8: Physical parameters, log scale
    # ------------------------------------------------------------
    plt.figure(figsize=(12, 7))
    for c in param_cols:
    ...
    plt.title("S7_C4 warm continuation: dynamic/gain parameters on log scale")
    plt.legend(loc="best", fontsize=8)
    ...
    plt.title("S7_C4 warm continuation: fitted theta offsets")
    plt.legend(loc="best")
    savefig(FIG_DIR / "09_theta_offsets.png")

    # ------------------------------------------------------------
    # Plot 10: Voltage-fit trust classification
    # ------------------------------------------------------------
    ...
    plt.xlabel("Cycle index")
    plt.ylabel("Best RMSE [V]")
    plt.title("S7_C4 warm continuation: voltage-fit quality classification")
    plt.legend(loc="best")
    ...
    print()
    print("=" * 80)
    print("S7_C4 TRUSTED REVIEW COMPLETE")
    print("=" * 80)
