...
SUMMARY_CSV = PROJECT_DIR / "results/tables/real_warm_continuation_ctid/S7_C4/all_cycles_summary.csv"
BEST_CSV = PROJECT_DIR / "results/tables/real_warm_continuation_ctid/S7_C4/all_cycles_best_runs.csv"

OUT_DIR = PROJECT_DIR / "results/figures/real_warm_continuation_ctid/S7_C4/trusted_diagnostics"
TABLE_OUT_DIR = PROJECT_DIR / "results/tables/real_warm_continuation_ctid/S7_C4/trusted_diagnostics"
...
def add_quality_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["flag_rankX_collapsed"] = df["best_rank_X_raw"] <= 1
...
def main() -> None:
    ...
    summary.to_csv(TABLE_OUT_DIR / "s7_trusted_summary_with_flags.csv", index=False)
    best.to_csv(TABLE_OUT_DIR / "s7_best_runs_copy.csv", index=False)

    # ------------------------------------------------------------
    # Plot 1: Best RMSE only, linear scale
    # ------------------------------------------------------------
    ...
    plt.title("S7_C4: best RMSE across 100 cycles")
    savefig("01_best_rmse_linear.png")

    # ------------------------------------------------------------
    # Plot 2: Best and median RMSE, log scale
    # This is better than the original because median spikes do not destroy the axis.
    # ------------------------------------------------------------
    plt.figure(figsize=(12, 6))
    ...
    plt.title("S7_C4: best vs median RMSE across cycles")
    plt.legend(loc="best")
    ...
    plt.xlabel("Cycle index")
    plt.ylabel("Best BFR [%]")
    plt.title("S7_C4: best BFR across cycles")
    plt.legend(loc="best")
    ...
    plt.xlabel("Cycle index")
    plt.ylabel("Best R2 [%]")
    plt.title("S7_C4: best R2 across cycles")
    plt.legend(loc="best")
    ...
    plt.title("S7_C4: successful and failed local starts per cycle")
    plt.legend(loc="best")
    ...
    plt.title("S7_C4: raw state and output-feature ranks")
    plt.legend(loc="best")
    ...
    plt.grid(True, alpha=0.35)
    plt.xlabel("Cycle index")
    plt.title("S7_C4: quality flags by cycle")
    plt.legend(loc="upper right", fontsize=8)
    savefig("07_quality_flags.png")

    # ------------------------------------------------------------
    # Plot 8: physically important parameters, raw value plots
    # Separate plots are better than one normalized mess.
    # ------------------------------------------------------------
    param_cols = [
        "alpha_n_hat",
        "alpha_p_hat",
    ...
    for col in available:
        ...
        plt.xlabel("Cycle index")
        plt.ylabel(col)
        plt.title(f"S7_C4: {col} across cycles")
        savefig(f"08_param_{col}.png")

    # ------------------------------------------------------------
    # Plot 9: log-scale positive parameters
    # ------------------------------------------------------------
    positive_params = [
        c for c in [
            "alpha_n_hat",
    ...
    plt.title("S7_C4: positive physical parameters, log scale")
    plt.legend(loc="best", fontsize=8)
    savefig("09_positive_parameters_log_scale.png")

    # ------------------------------------------------------------
    # Plot 10: theta offsets only
    # ------------------------------------------------------------
    ...
    plt.title("S7_C4: fitted theta offsets")
    plt.legend(loc="best")
    savefig("10_theta_offsets.png")

    # ------------------------------------------------------------
    # Plot 11: best RMSE with bad cycles highlighted
    # ------------------------------------------------------------
    plt.figure(figsize=(12, 6))
    plt.semilogy(summary["cycle_index"], summary["best_rmse"], marker="o", linewidth=2.2, label="best RMSE")
    ...
    plt.title("S7_C4: RMSE with bad-voltage and rank-collapse flags")
    plt.legend(loc="best")
    ...
    rankx_cycles = summary.loc[summary["flag_rankX_collapsed"], "cycle_index"].astype(int).tolist()
    many_fail_cycles = summary.loc[summary["flag_many_failures"], "cycle_index"].astype(int).tolist()

    report_path = TABLE_OUT_DIR / "s7_trusted_diagnostic_report.txt"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("S7_C4 trusted diagnostic report\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"Rank-X collapsed cycles: {rankx_cycles}\n\n")
